...
export default function NarrativeHeader() {
  return (
    <header className="relative max-w-4xl">
      ...
      >
        <span className="inline-flex h-2 w-2 rounded-full bg-phase-settled shadow-[0_0_10px_rgba(16,185,129,0.8)]" />
      ...
      >
        ...
        <span className="text-ink-0">Both legs, or neither.</span>
      </motion.h1>
      ...
      <motion.div
        ...
        className="mt-6 space-y-3 text-ink-1 font-sans text-lg leading-relaxed max-w-2xl"
      >
        ...
        <p>
          An <span className="text-phase-active font-medium">RFQ</span> — request for quote — is a
          two-party trade: a <span className="text-ink-0">taker</span> asks for a price, a{' '}
          <span className="text-ink-0">maker</span> quotes one, and the two assets change hands.
        </p>
        <p>
          The settlement is <span className="text-phase-settled font-medium">atomic</span>: both legs
        ...
        <p className="text-ink-1">
          That is the mechanism — <span className="text-ink-0">atomic settlement, enforced on-chain</span>. Below is a
          real one, replayed step by step from a <span className="text-ink-0">controlled Supra Mainnet RFQ proof
          with separated roles</span>: a SupraOS-side requesting agent (the demand side) initiates the RFQ,{' '}
          <span className="text-phase-active font-medium">Kahless</span> represents the Maker side, and behind Kahless{' '}
          <span className="text-phase-proof font-medium">K1</span> acts as the bonded Maker-Operator.{' '}
          <span className="text-phase-settled font-medium">COSMO</span> locks, checks and settles the exchange atomically
