...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-baseline justify-between gap-3">
      ...
      <span className="font-mono text-xs text-ink-0">{value}</span>
    </div>
  );
}
    ...
    <div className="rounded-xl border border-line-base bg-surface-1 p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          />
          <div>
            <div className="font-mono text-sm font-bold text-ink-0">{op.label}</div>
            <div className="font-mono text-[11px] text-ink-2">{op.role}</div>
          </div>
        </div>
        <span className="font-mono text-[11px] text-ink-2">{shortAddr(op.addr)}</span>
      </div>
