...
# Aufruf: scripts/set-intelligence-payment-url.sh https://buy.stripe.com/...
set -euo pipefail
cd "$(dirname "$0")/.."
URL="${1:-}"
[[ "$URL" =~ ^https://[^[:space:]\"]+$ ]] || { echo "Aufruf: $0 https://..."; exit 1; }
node -e '
const fs=require("fs");const p="src/data/intelligence-offer.json";
const o=JSON.parse(fs.readFileSync(p,"utf8"));o.payment_url=process.argv[1];
fs.writeFileSync(p,JSON.stringify(o,null,2)+"\n");' "$URL"
[ -d out.pre-intel-payment ] || cp -r out out.pre-intel-payment
